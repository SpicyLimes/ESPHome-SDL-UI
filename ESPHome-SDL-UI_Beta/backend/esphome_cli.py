...
from .config import ESPHOME_VENV_PATH
from .logger import setup_logger

logger = setup_logger(__name__)

class ESPHomeCLI:
    def __init__(self, venv_path: str = ESPHOME_VENV_PATH):
        self.venv_path = Path(venv_path)
        self.esphome_bin = self.venv_path / "bin" / "esphome"
        self.current_process: Optional[asyncio.subprocess.Process] = None

    def _get_command_prefix(self) -> List[str]:
        return [str(self.esphome_bin)]

    async def validate_config(self, config_path: Path) -> Tuple[bool, str]:
        try:
    ...
    async def compile_config(self, config_path: Path) -> Tuple[bool, str]:
        try:
