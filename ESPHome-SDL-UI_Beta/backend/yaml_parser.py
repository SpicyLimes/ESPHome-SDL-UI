...
class ESPHomeSecretConstructor:
    """Custom YAML constructor for ESPHome !secret tag"""
    @staticmethod
...
class YAMLParser:
    def __init__(self):
        ...
        yaml.SafeLoader.add_constructor('!secret', ESPHomeSecretConstructor.secret_constructor)
    ...
    def load_yaml(self, file_path: Path) -> Tuple[bool, Optional[Dict[str, Any]], Optional[str]]:
        try:
            ...
            if len(substitutions_sections) > 1:
                ...
                merged_content = '\n'.join(lines)
                self.config_data = yaml.safe_load(merged_content)
            else:
